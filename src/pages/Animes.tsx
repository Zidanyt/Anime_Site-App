...
const Animes = () => {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const favoriteIds = favData.map((anime: Anime) => anime.id);
        setAnimes(animesData);
        setFavorites(favoriteIds);
        setFiltered(animesData);
      } catch (err) {
  ...
  return (
    <div className="animes-page">
      ...
      {/* Exibe mensagem se nenhum anime for encontrado */}
      {filtered.length === 0 && (
        <p className="no-results">Nenhum anime encontrado com essa busca.</p>
      )}
        ...
        {filtered.map((anime) => {
          ...
          const avgRating =
            anime.ratings && anime.ratings.length > 0
              ? anime.ratings.reduce((sum, r) => sum + r.score, 0) /
                anime.ratings.length
              : 0;
